...
void gray_convert(char *file_path){
	...
	int output_bmp = open("output_bmp.bmp",  O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	
	if(output_bmp == -1){
		error("Nu s-a putut crea fiserul output_bmp.bmp");
	}
	
	write(output_bmp, header, sizeof(header));
	...
	for(int i=0; i < size; i++){
		read(input_file, pixel, sizeof(pixel));
		unsigned char gray_pixel = 0.299 * pixel[0] + 0.587 * pixel[1] + 0.114 * pixel[2];
		pixel[0] = gray_pixel;
        	pixel[1] = gray_pixel;
        	pixel[2] = gray_pixel;
	
		write(output_bmp, pixel, sizeof(pixel));
	}
	close(input_file);
	close(output_bmp);
}
